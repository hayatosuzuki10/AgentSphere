...
/**
 *　Hubを管理するためのクラス
 * 　1ノードに同一種類かつ複数のHubを管理するためにMAPで種類別に管理、更にその内部をListで保持している。
 * @author kousuke
 */
public final class HubContainer {
    ...
    private Node directSuccessor;
    
    private Node directPredecessor;
    
    private Address addressOfLocalNode;
    
    private boolean isMainNode = false;
    
    private List<Node> accessList ; 
    
    private ArrayList<Node> crossHubLink;
    ...
    public HubContainer(int divideScore) {
        ...
        this.crossHubLink = new ArrayList<Node>();
        this.accessList= new ArrayList<Node>();
    ...
    public void addCrossHubLink(Node proxy){
        this.crossHubLink.add(proxy);
    }
    
    public Node getCrossHubLinkRandomly(){
    ...
    public final Node[] createHubNetwork(Address[] addressOfNodes,String typeOfHub){
        Node[] Nodes=null;
         ...
         * @param keyTocheck　探しているデータの範囲
         * @return 完全に範囲内の場合＝０、完全に範囲外の場合=-1。範囲内＆範囲以上＝１。範囲内＆範囲以下≒２,範囲内だが最小値、最大値ともにハミ出た場合＝３、詳しくはドキュメント参照
         */
     public final int isRange(DataRange keyTocheck){
         if(this.hubContainer.containsKey(keyTocheck.getType())){
             LinkedList<Hub> checkingList = this.hubContainer.get(keyTocheck.getType());
     ...
     /**
      * 担当範囲を分割して返す
      * このメソッドを使う前にisRangeメソッドを利用してIndexを調べる必要がある。
      * @param key　Hubの種類を表すKey
      * @param index 複数Hubを保持している場合のIndex
      * @return  {@link DataRange}によって分割されてできた新しいRange
      */
     public final DataRange divideRange(String key,int index){
         DataRange newRange = hubContainer.get(key).get(index).divideRange(divideScore);
         return newRange;
     }
      ...
      * @param pair　　
      */
     public final void setEntry(String type,int index,ArrayList<KeyValuePair> pairs ){
     ...
     /**
      * 保存してあったエントリを取得するためのメソッド
      * このメソッドを使う前にisRangeメソッドを利用してIndexを調べる必要がある。
      * @param range　取得するデータの範囲
      * @param index　取得するデータを保持しているINDEX
      ...
      * @return 
      */
     public final Hub getHubRandomly(Node fromNode){
...
//        randomHub.addSuccessor(randomHub.getDirectSuccessor());
     ...
     public final int getDataRangeWithInteger(String typeOfHub){
         if(this.hasHub(typeOfHub)){
             return this.hubContainer.get(typeOfHub).get(0).getRange().caluculateDifference();
         }else{
                 RuntimeException e = new RuntimeException("this node hasnt successor hub! somewhere seriously wrong!");
                 logger.fatal(e);
                 throw e;
        }
     }
    
     public final void setAccessLink(Node accessLink){
      ...
      */
     public final Node getAccessLinkRandomly(){
         Random random  = new Random();
