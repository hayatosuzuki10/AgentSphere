...
public class MSSlave4 extends AbstractAgent implements IMessageListener {
	...
	int startpoint = 0;
	int slaveNum = 0;
	String[] ANlist = new String[5];

	long[] pack = new long[4];
	int[][] pack2_1 = new int[2][mHeight*mWidth];
	...
	KeyValuePair<InetAddress, Integer> slave3 = null;
	KeyValuePair<InetAddress, Integer> slave4 = null;
 ...
 //ここから
	public void run(){
		...
		try {
			...
			slave3 = new KeyValuePair<InetAddress, Integer>(Inet4Address.getByName(IPAddress.Slave3),55878);
			slave4 = new KeyValuePair<InetAddress, Integer>(Inet4Address.getByName(IPAddress.Slave4),55878);
		...
		while(gamePlay){
			...
			if(turnCount%10000 == 0) {
				while(loopflag) {
					loopCount++;
					if(sendNum == slaveNum && sendflag) {
						BoardShare(slaveNum);
						shareCount++;
					}else if(sendNum == 4 && sendflag == false) {
						sendNum = 1;
						sendflag = true;
						loopflag = false;
					}
		...
		pack[0] = CommunicateTime;

		System.out.println("S"+slaveNum+": Sending -> M");

		pack[3] = System.currentTimeMillis();

        StandardEnvelope envelope = new StandardEnvelope(new AgentAddress(ANlist[0]), new StandardContentContainer(pack));
...
*/
		...
		System.out.println("S"+slaveNum+": Sending -> other S");
        StandardEnvelope envelope; // = new StandardEnvelope(new AgentAddress(IPAddress.Slave1), new StandardContentContainer(pack2));

		if(slaveNum != 1){
			envelope = new StandardEnvelope(new AgentAddress(ANlist[1]), new StandardContentContainer(pack2_1));
			tempTime1 = System.currentTimeMillis();
			MessageAPI.send(slave1, envelope);
			tempTime2 = System.currentTimeMillis();
			CommunicateTime += (tempTime2-tempTime1);

		}
		if(slaveNum != 2) {
			envelope = new StandardEnvelope(new AgentAddress(ANlist[2]), new StandardContentContainer(pack2_1));
			tempTime1 = System.currentTimeMillis();
			MessageAPI.send(slave2, envelope);
			tempTime2 = System.currentTimeMillis();
			CommunicateTime += (tempTime2-tempTime1);

		}
		if(slaveNum != 3) {
			envelope = new StandardEnvelope(new AgentAddress(ANlist[3]), new StandardContentContainer(pack2_1));
			tempTime1 = System.currentTimeMillis();
			MessageAPI.send(slave3, envelope);
			tempTime2 = System.currentTimeMillis();
			CommunicateTime += (tempTime2-tempTime1);

		}
		if(slaveNum != 4) {
			envelope = new StandardEnvelope(new AgentAddress(ANlist[4]), new StandardContentContainer(pack2_1));
			tempTime1 = System.currentTimeMillis();
			MessageAPI.send(slave4, envelope);
			tempTime2 = System.currentTimeMillis();
			CommunicateTime += (tempTime2-tempTime1);

		}

		System.out.println("S"+slaveNum+": Sent -> other S");
		System.out.println();
	  ...
	  public void openSquare(){
		  ...
		  for(int i = 0 ; i < HintList.size() ; i++){
			  for(int j = 0 ; j < HintList.get(i).hintSquare.size() ; j++){
				  if(!mineFlag[HintList.get(i).hintSquare.get(j)]) {
					  int temp = HintList.get(i).hintSquare.get(j);
					  openFlag[temp] = true;
	...
	public void receivedMessage(AbstractEnvelope envelope) {
		...
		System.out.println("S"+slaveNum+": Received Message <- other S");
	...
	public void receivedMessage(AbstractEnvelope envelope, int flag) {
		...
		/*
			...
			StandardContentContainer cont = (StandardContentContainer) envelope.getContent();
			pack2_2=(int[][]) cont.getContent();

			List<Hint> tempHint = new ArrayList<Hint>();
			int HLsize = pack2_2[0][0];

			for(int i=0 ; i<HLsize ; i++) {
				Hint h = new Hint();
				h.mNum = pack2_2[i+1][0];
				for(int j=0 ; j<pack2_2[i+1][1] ; j++) {
					h.hintSquare.add(pack2_2[i+1][j+2]);
				}
				tempHint.add(h);
			}
			//tempHint = pack2[3~][]
			...
			for(int i=0 ; i<mHeight*mWidth ; i++) {
				if(openFlag[i] || mineFlag[i]) {
					for(int j=0 ; j < tempHint.size() ; j++){
						  for(int k=0 ; k < tempHint.get(j).hintSquare.size() ; k++){
							  if(tempHint.get(j).hintSquare.get(k) == i){
								  tempHint.get(j).hintSquare.remove(k);
								  if(mineFlag[i] == true) tempHint.get(j).mNum--;
							  }
						  }
					  }
				}
			...
			HintList.addAll(tempHint);
			List<Hint> NewHint = new ArrayList<Hint>(new HashSet<>(HintList));
			HintList.clear();
			HintList = NewHint;
		}
		*/
