...
public class DL4JtestMNIST5LayerA extends AbstractAgent { 

    private static Logger log = LoggerFactory.getLogger(DL4JtestMNIST5LayerA.class);
    
    void myfit(MultiLayerNetwork model, int numEpochs,int batchSize){
	...
	public @continuable void run() {
		...
		myfit(model, nEpochs, batchSize);
		/*model.setListeners(new ScoreIterationListener(10),
				new EvaluativeListener(mnistTest, 1, InvocationType.EPOCH_END)); // Print score every 10 iterations and
																					// evaluate on test set every epoch
		model.fit(mnistTrain, nEpochs);*/

		String path = "C:/DeepLearning4JExamples/Otameshi4.zip";//FilenameUtils.concat(System.getProperty("java.io.tmpdir"), "Otameshi4lenetmnist.zip");
