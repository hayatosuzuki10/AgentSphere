...
public class MSMaster4old extends AbstractAgent implements IMessageListener{
	...
	boolean[] sCheck = {false, false, false, false};


	 MSSlave4old slave1=new MSSlave4old();
	 MSSlave4old slave2=new MSSlave4old();
	 MSSlave4old slave3=new MSSlave4old();
	 MSSlave4old slave4=new MSSlave4old();
	...
	public void run(){
		...
		try {
		  ...
		  //各Slaveに送る
		  System.out.println("M: Sending -> all S");
		  StartTime = System.currentTimeMillis();
		  ANlist[0] = getStrictName();
		  ANlist[1] = slave1.getStrictName();
		  ANlist[2] = slave2.getStrictName();
		  ANlist[3] = slave3.getStrictName();
		  ANlist[4] = slave4.getStrictName();


		  slave1.setattr(mMinefield, mineNum, ANlist, 0, 1);
		  slave2.setattr(mMinefield, mineNum, ANlist, (mWidth*mHeight-1), 2);
		  slave3.setattr(mMinefield, mineNum, ANlist, (mWidth-1), 3);
		  slave4.setattr(mMinefield, mineNum, ANlist, (mWidth*(mHeight-1)), 4);
		  ...
		  System.out.println("M: Sent -> all S");
		  System.out.println();
...
*/
	...
	public void receivedMessage(AbstractEnvelope envelope) {
		...
		if(pack2_1[0][mHeight*mWidth] == 0) {
			...
			System.out.println("M: Received Message <- S"+pack2_1[0][mHeight*mWidth]);
			for(int i=0 ; i<mHeight*mWidth ; i++) {
				if(pack2_1[0][i] == 0) openFlag[i] = true;
				if(pack2_1[1][i] == 0) mineFlag[i] = true;
			}

			sCheck[pack2_1[0][mHeight*mWidth]-1] = true;

			if(sCheck[0] && sCheck[1] && sCheck[2] && sCheck[3]) {
				for(int i=0 ; i<mHeight*mWidth ; i++) {
