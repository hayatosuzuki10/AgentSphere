...
public class MSSlave2 extends AbstractAgent implements IMessageListener {
			...
			if(turnCount%10000 == 0) {
				while(loopflag) {
					...
					if(sendNum == slaveNum && sendflag) {
						BoardShare(slaveNum);
						shareCount++;
					}else if(sendNum == 2 && sendflag == false) {
						sendNum = 1;
						sendflag = true;
						loopflag = false;
					}
...
*/
		...
		if(slaveNum != 1){
			...
			//envelope = new StandardEnvelope(new AgentAddress(ANlist[1]), new StandardContentContainer(pack2_2));
			//MessageAPI.send(slave1, envelope, false);
		}
		if(slaveNum != 2) {
			...
			tempTime2 = System.currentTimeMillis();
			CommunicateTime += (tempTime2-tempTime1);
			//envelope = new StandardEnvelope(new AgentAddress(ANlist[2]), new StandardContentContainer(pack2_2));
			//MessageAPI.send(slave2, envelope, false);
		}
